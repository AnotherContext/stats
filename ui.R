...
shinyUI(pageWithSidebar(
  ...
  # Show a tabset that includes  interactive plots and tables in separate tabs
  ...
  "), br(), br(), ("function of your choice >> you can type your own function in the additional text field that will pop out after choosing this option. As a variable, use either 'x' or  type '1:N'. You could also add some noise 'rnorm(N)' and/or random walk 'cumsum(rnorm(N))'."), br(), br(), br(), em(">> Fitting Type >> Parametric Model"), br(), ("There are 2 options: you can leave fitting to the program or fit harmonics by choosing 'User defined' option, and then typing sine and cosine components into appopriate text fields. Use 'time.predict' as your variable."), br(), br(), strong("All data are updated immediately after the inputs change."), HTML("<hr />"), ("For more detailed explanation and conclusions, please refer to instractions or project report which is soon going to be available for download.")),
